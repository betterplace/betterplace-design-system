import { Meta } from '@storybook/addon-docs'

<Meta title="Guides/Manual testing guide" />

# Manual testing guide

🚧 Add general testing guidelines

## Testing accessibility

- Test with a **pointing device** (e.g. a computer mouse) and **touch device** to ensure interactive elements work.
- Test responsiveness and touch usability on **mobile** devices.
- **Zoom** in and out (ctrl +/-) and change the font size in your browser settings to ensure content is accessible outside your standard view.
- Test with **only a keyboard** to ensure all content is accessible without using a pointing or touch device. ([Intro to keyboard testing](https://webaim.org/techniques/keyboard/))
- Test with a **screen reader** (learn how below) to ensure all content is accessible and coherent when read aloud.
- If content changed / has been added: Read through the **content** to ensure everything is understandable.
- If design changed / is new: Emulate **color vision deficiencies** (learn how below) to ensure content is distinguishable.

### How to test with a screen reader

According to a [survey](https://webaim.org/projects/screenreadersurvey9/) conducted by WebAIM, more than 90% of screen reader users reported using a screen reader on Windows. The most popular screen readers are

- JAWS (Windows, can run in free demo mode for 40 minutes at a time)
- NVDA (Windows, free) and
- VoiceOver (Mac, free).

#### Basics

[Intro to using a screen reader](https://tetralogical.com/blog/2021/09/29/browsing-with-a-desktop-screen-reader/)

tbd

#### How to test with JAWS

JAWS works best with Chrome and Firefox. When testing, pair it with Chrome.

tbd

#### How to test with NVDA

NVDA works best with (and should, therefore, be paired with) Firefox.

tbd

#### How to test with VoiceOver

VoiceOver works best with (and should, therefore, be paired with) Safari.

tbd (No installation needed; some helpful settings can be made).

Find information on how to test on touch devices with screen readers here: [Summary of screen reader testing on Android and iOS](https://developer.mozilla.org/en-US/docs/Learn/Accessibility/Mobile#summary_of_screen_reader_testing_on_android_and_ios)

### How to test for color vision deficiencies

#### In Firefox

- Open _Accessibility_ panel.
- Choose from a selection of different color vision deficiencies in the _Simulate_ drop-down.

<img src="color_vision_deficiencies_settings_firefox.png" alt="Color vision deficiencies settings in Firefox" width="50%" />

#### In Chrome

- Open the _Rendering_ panel (either use the command menu and type _rendering_ or use the three-dot menu in the top-right corner of the screen and go to _More Tools_ to find it).
- Choose from a selection of different color vision deficiencies in the _Emulate vision deficiencies_ drop-down.

<img src="color_vision_deficiencies_settings_chrome.png" alt="Color vision deficiencies settings in Chrome" width="50%" />
